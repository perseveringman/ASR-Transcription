...
**深度解析**：...`;
    ...
    private getBookRecommendationPrompt(): string {
        return `你是一个博学的阅读顾问，致力于打破信息茧房。请根据用户的笔记内容，推荐 4 本书籍。
**策略**：
1.  前 3 本：**深度相关**，深化或扩展笔记中的观点（侧重思想深度和经典性）。
2.  第 4 本：**惊喜跨界 (Serendipity)**，推荐一本看似与主题完全无关（如不同学科、不同领域），但在底层逻辑或隐喻上能带来奇妙启发的书。
...
4. **《[书名]》** [作者] (💥惊喜跨界)
   ...
   *   **核心洞察**：...`;
    ...
    private getPoetryRecommendationPrompt(): string {
        return `你是一个文学鉴赏家。请根据用户的笔记意境，寻找 3 首**现存的经典诗歌**。
**策略**：
1.  前 2 首：**同频共鸣**，风格与笔记的情感基调相符。
2.  第 3 首：**反差冲击**，选择一首风格、时代或情感基调截然相反的诗（例如：若笔记忧郁，则推豪放；若笔记理性，则推狂野），用对立面来激活感受。
...
3. **《[诗名]》** [作者] (⚡️反差)
   ...
   *   **冲击赏析**：[这首诗以完全不同的...风格，打破了...的沉闷/惯性，带来了...]`;
    ...
    private getFigureRecommendationPrompt(): string {
        return `你是一个传记作家。请根据用户的笔记内容，推荐 3 位人物。
**策略**：
1.  前 2 位：**思想契合**，历史或现代领域内的高度相关人物。
2.  第 3 位：**跨界连接**，一位完全不同领域（如艺术 vs 科学，古代 vs 现代）的人物，但其思维模式或人生选择有惊人的相似或互补之处。
...
3. **[人物姓名]** ([身份/时期]) (🔀跨界)
   ...
   *   **启发**：...`;
    ...
    private getMediaRecommendationPrompt(): string {
        return `你是一个文化策展人。请根据用户的笔记内容，推荐 3 部电影、纪录片或播客。
**策略**：
1.  前 2 部：**主题印证**，从侧面深化笔记内容。
2.  第 3 部：**脑洞大开**，一部风格迥异、题材跳跃甚至怪诞的作品，旨在提供完全不同的思考维度或审美体验。
